...
| `PermissionDenied` | `tool_result` `isError: true` |
| `SessionEnd` | `end`, `stop_reason: "user"` |
| `StopFailure` | `end`, `stop_reason: "error"` |

**`Stop` is not registered in 0.1.** It is a turn boundary, not session end. `stop_hook_active` is not copied onto evidence.

Each entry is a `type: "command"` hook:
...
```json
{
  ...
  "command": "node",
  "args": [
    "<absolute-path-to-bin/session-contract.js>",
    "cc-hook",
    "<Event>"
  ]
}
...
No shell wrapper (`sh -c`, `cmd /c`). Windows and POSIX both exec `node` with an argv vector. The bin path is the installed `bin/session-contract.js` (resolved from the package), **not** `process.execPath`.
...
The argv event name MUST equal stdin `hook_event_name`. Mismatch → no write, exit 1, stderr.
...
**Idempotent merge:** if an existing command-hook for that event already has `args` containing `cc-hook` and the same event name, leave it. Do not delete other plugins’ hooks. Do not duplicate ours. **Do not remove or rewrite other top-level keys** (`permissions`, `env`, `model`, …).
...
9. `cc-init --print` JSON parses and contains the seven event names and `cc-hook` argv.
