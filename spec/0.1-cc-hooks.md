...
|---|---|---|---|---|
| **2.1.87 (measured)** | Shell `command` string. `args` exec form is stripped → dead hook if that is all we send. | **Safe (core).** In the product event enum. `SessionStart`+C → Found 1. | **Safe (core).** In the product event enum. `SessionStart`+C → Found 1. | **Type-0 poison.** Not in the enum (`PermissionRequest` is the pre-decision event). `SessionStart`+C → **Found 0** (all matchers gone). Default `cc-init` omits it. `--full` is for current-docs products only. Deny without this event: unmatched `tool_call`. |
| **Current docs** | `command` is the shell string; **`args` present ⇒ exec form.** We still emit shell form only. | Present. | Present. | Present. `--full` registers it; mapping stays adapter §3.3. |
