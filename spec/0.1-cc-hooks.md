...
Facts below match the Hooks reference ([code.claude.com/docs/en/hooks](https://code.claude.com/docs/en/hooks)) plus **measured** 2.1.87 (Windows) and 2.1.237 (Linux) binaries. `settings.json` `hooks` is `{ "<Event>": [{ "matcher": "...", "hooks": [{ "type": "command", "command": "<shell string>", "timeout"? }] }] }`. Empty `matcher` matches all. Matched hooks run **in parallel**, one process per event, JSON on stdin. **exit 2 with no valid JSON blocks** (stderr fed to Claude); 0/1 do not block; default timeout 60s; **SessionEnd is capped at 1.5s**; hook JSON has **no timestamp**.

**Poison (2.1.87 measured):** an **unknown event key** in `hooks` (notably `PermissionDenied`) is **not** ignored. The product drops **every** matcher (SessionStart query `Found 0 hook matchers`). A fragment that registers a poison key is worse than registering nothing, and would also zero **other plugins’** hooks in the same file. `cc-init` default therefore emits only the **core** set. `cc-hook` still *handles* all seven events (`--full` is how a current-docs product opts into the extra keys). **No version probe**; the matrix below replaces one.

The debug line `Found N hook matchers in settings` counts matchers for the **event currently firing**, not the number of keys in the file. At `SessionStart` / `source: startup`, N is 1 when that event is registered and the blob parsed, 0 when a poison key voided the blob.

## D1. Registration

0.1 writer can handle seven events. **Default `cc-init` registers core only.**

| Event | Set | Evidence (adapter §3.3) |
|---|---|---|
| `SessionStart` | core | `start` |
| `PreToolUse` | core | `tool_call` |
| `PostToolUse` | core | `tool_result` (or `tool_call`+`tool_result` if no prior call) |
| `PostToolUseFailure` | core | `tool_result` `isError: true` |
| `SessionEnd` | core | `end`, `stop_reason: "user"` |
| `StopFailure` | core | `end`, `stop_reason: "error"` |
| `PermissionDenied` | optional (`--full`) | `tool_result` `isError: true` |

**`Stop` is not registered.** Turn boundary, not session end. **`PermissionRequest` is not registered.** It fires *before* a permission decision and is not deny evidence.

Without `--full`, a denied tool degrades: `PreToolUse` already wrote `tool_call`; no result → `check` emits `warning.unmatched_tool_call`. Honest “attempted, unresolved.” Do not invent an `isError` result.
...
No version probe. Isolation on 2.1.87: `SessionStart` + candidate C; SessionStart query `Found 1` = C did not void the blob; `Found 0` = type-0 poison.

| Product | Core six | `--full` (`PermissionDenied`) |
|---|---|---|
| **2.1.87 (Windows, measured)** | All six are in the product enum (safe to register). | **Poison:** registering it zeroes every matcher. |
| **2.1.237 (Linux, measured)** | All six actually fired: `SessionStart`, `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `SessionEnd`, `StopFailure`. | Registration does **not** poison matchers. A sandbox block does **not** emit `PermissionDenied` (denied attempts still appear as a dangling `tool_call`). Ask-mode human deny is untested. |

On 2.1.237 a `--full` session that blocked `cat /etc/passwd` produced `start → tool_call(Bash) → end`. The deny path is unmatched `tool_call`, not a `PermissionDenied` event. Default `cc-init` remains **core-only**; `--full` is still an explicit opt-in.

Command-shape facts used to pick the shell form:

| | command shape | `PostToolUseFailure` | `StopFailure` | `PermissionDenied` |
|---|---|---|---|---|
| **2.1.87 (Windows, measured)** | Shell `command` string. `args` exec form is stripped → dead hook if that is all we send. | **Safe (core).** In the product event enum. `SessionStart`+C → Found 1. | **Safe (core).** In the product event enum. `SessionStart`+C → Found 1. | **Type-0 poison.** Not in the enum (`PermissionRequest` is the pre-decision event). `SessionStart`+C → **Found 0** (all matchers gone). Default `cc-init` omits it. Deny without this event: unmatched `tool_call`. |
| **2.1.237 (Linux, measured)** | `command` is the shell string; **`args` present ⇒ exec form.** We still emit shell form only. | Fired (core). | Fired (core). | In the enum (not poison). Sandbox deny still does **not** emit this event; mapping if it did fire stays adapter §3.3. |
...
| Flag | Behaviour |
|---|---|
| *(none)* / `--print` | **Default.** Stdout = JSON object `{ "hooks": { ... **core** events ... } }` and nothing else. Does not touch disk. Never includes poison keys. |
| `--full` | Include optional events (`PermissionDenied`) in the fragment. Combine with `--print` or `--write`. Do **not** use on 2.1.87 (type-0 poison). |
...
**Idempotent merge:** if an existing command-hook for that event already has a `command` string containing `cc-hook <Event>`, leave it. A previous session-contract **args-form** entry for that event is **replaced** by the shell-form command. Default (no `--full`) **removes our optional-event entries** (and deletes the event key when nothing else remains) so a prior `--full` write cannot keep poisoning 2.1.87. Foreign hooks on that key are left (we do not delete other plugins’ hooks). Do not duplicate ours. **Do not remove or rewrite other top-level keys** (`permissions`, `env`, `model`, …).
...
9. `cc-init --print` JSON is the **core** set (no poison keys); each `command` string contains `cc-hook <Event>`. `--full` adds optional events.
